...
# 线性层：nn.Linear(1, 1) ，它输入输出都是一维张量
# weight ：  权重   ；  bias ： 偏置
layer = nn.Linear(1, 1)
print(f'layer: weight = {layer.weight.item():.8f}, bias = {layer.bias.item():.8f}')
layer(torch.tensor([[1.0],[2.]]))
print(layer(torch.tensor([[1.0],[2.]])))

# 3. 定义模型
class SimpleNet(nn.Module):
    def __init__(self):
        super().__init__()
        # (1,1)的线性层实际上内部就是一个线性方程，对于输入任何数字x，它会输出x * w + b，实际上神经网络中的“layer”就是内含了一系列参数、可被训练的单元
        self.linear = nn.Linear(1, 1)  # 输入维度=1，输出维度=1 都是一维张量
