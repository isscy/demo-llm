...
# 基于嵌入层（Embedding）和线性层（Linear）来学习字符级别的序列预测任务，与之前的二元模型类似，但引入了深度学习框架：使用神经网络学习字符嵌入表示，并通过交叉熵损失训练模型。模型可以生成文本序列，类似于GPT的简化版
...
class BabyGPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, n_embd: int):
        super().__init__()
        # 嵌入层，将离散索引（词汇 ID）映射到连续向量。num_embeddings 是词汇大小，embedding_dim 是嵌入维度（这里是 n_embd）。内部存储一个可学习的权重矩阵 (vocab_size, n_embd)
        self.token_embedding_table = nn.Embedding(vocab_size, n_embd)  # 嵌入层，把token映射到n_embd维空间
        # 线性层（全连接层），计算 output = input @ weight.T + bias。 in_features 是输入维度（n_embd），out_features 是输出维度（vocab_size）。包含可学习权重和偏置
        self.lm_head = nn.Linear(n_embd, vocab_size)  # 线性层，把n_embd维空间映射到vocab_size维空间，

    # 前向传播。训练时计算损失；推理时只返回 logits
    ...
    # 从初始序列 idx 开始，基于模型预测逐步生成新 token
