...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        wei = q @ k.transpose(-2, -1) / (
                    k.size(-1) ** 0.5)  # (B, T, head_size) @ (B, head_size, T) = (B, T, T)，最后缩放避免softmax过于稀疏
...
for iter in range(max_iters):
    ...
    if iter % eval_interval == 0:
        ...
        print(
            f"step {iter}: train loss {losses['train']:.4f}, val loss {losses['val']:.4f}, speed: {tokens_per_sec:.2f} tokens/sec, time: {int(elapsed_mins)}m {elapsed_secs:.1f}s")
